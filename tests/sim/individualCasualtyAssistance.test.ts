...
describe("individual casualty assistance and sparse drag groups", () => {
  ...
  it("keeps the gathering tick-start projection after same-tick energy reduction", () => {
    const simulation = createSimulation(scenario([
      unit(1, 1, 100),
      {
        ...unit(2, 1, 160),
    ...
    advanceSimulationOneTick(simulation);
    expect(beforeNextX - simulation.world.positionsX[1]!).toBe(2);
    expect(getIndividualEnergyActivityInspection(
      combat.individualEnergyActivityStore, 1,
    )).toMatchObject({
      requestedPhysicalGait: "sprinting",
  ...
  it("preserves direct one-Physick drag evidence through later activity observation", () => {
    const simulation = createSimulation(scenario([
      {
        ...unit(1, 1, 100),
        energyProfile: {
    ...
    const patientDelta = simulation.world.positionsX[0]! - before[0]!;
    const helperDelta = simulation.world.positionsX[1]! - before[1]!;
    expect(patientDelta).toBe(helperDelta);
    expect(Math.abs(helperDelta)).toBe(2);
    expect(getIndividualEnergyActivityInspection(
      combat.individualEnergyActivityStore, 1,
    )).toMatchObject({
  ...
  it("uses the slowest helper capability for the coherent two-fighter drag group regardless of helper order", () => {
    const run = (reverseActiveHelpers: boolean) => {
      ...
      simulation.world.positionsX[1] = 96;
      ...
      const deltas = [0, 1, 2].map(
        (entityId) => simulation.world.positionsX[entityId]! - beforeX[entityId]!,
      );
      const helperActivity = [1, 2].map((entityId) =>
        getIndividualEnergyActivityInspection(
      ...
      return {
        ...
        deltas,
        helperGait: helperActivity.map((activity) => ({
          requestedPhysicalGait: activity.requestedPhysicalGait,
          effectivePhysicalGait: activity.effectivePhysicalGait,
          actualPhysicalGait: activity.actualPhysicalGait,
          gaitReducedByCapability: activity.gaitReducedByCapability,
          physicalGaitSource: activity.physicalGaitSource,
          movementExpenditureRequested: activity.movementExpenditureRequested,
        })),
        patientActivity: getIndividualEnergyActivityInspection(
          combat.individualEnergyActivityStore, 0,
        ),
        helperEnergy: helperActivity.map(
          (activity) => activity.energyAfter,
        ).sort((left, right) => left - right),
        helperExpenditureApplied: helperActivity.map(
          (activity) => activity.expenditureApplied,
        ).sort((left, right) => left - right),
    ...
    expect(canonical.deltas[0]).toBe(canonical.deltas[1]);
    expect(canonical.deltas[1]).toBe(canonical.deltas[2]);
    expect(Math.abs(canonical.deltas[0]!)).toBe(1);
    expect(canonical.helperGait).toEqual([
      expect.objectContaining({
        requestedPhysicalGait: "jogging",
  ...
  it("applies drag expenditure to following-tick capability only", () => {
    const simulation = createSimulation(scenario([
      unit(1, 1, 100),
      {
        ...unit(2, 1, 104),
    ...
    group.destinationX = 20;
    group.destinationY = 60;
    const beforeFirstDrag = simulation.world.positionsX[1]!;

    advanceSimulationOneTick(simulation);
    expect(Math.abs(
      simulation.world.positionsX[1]! - beforeFirstDrag,
    )).toBe(2);
    expect(getIndividualEnergyActivityInspection(
      combat.individualEnergyActivityStore, 1,
    )).toMatchObject({
    ...
    advanceSimulationOneTick(simulation);
    expect(Math.abs(
      simulation.world.positionsX[1]! - beforeFollowingTick,
    )).toBe(1);
    expect(getIndividualEnergyActivityInspection(
      combat.individualEnergyActivityStore, 1,
    )).toMatchObject({
  ...
  it("preserves drag cadence remainder through energy reduction and world-bounded zero displacement", () => {
    const run = (startingEnergy: number, bounded: boolean) => {
      const simulation = createSimulation(scenario([
        unit(1, 1, 100),
        {
          ...unit(2, 1, 104),
      ...
      if (bounded) {
        simulation.world.positionsX[0] = 1;
        simulation.world.positionsX[1] = 0;
        simulation.world.positionsY[0] = 60;
        simulation.world.positionsY[1] = 60;
        group.destinationX = 0;
        group.destinationY = 60;
      } else {
        group.destinationX = 20;
        group.destinationY = 60;
      ...
      for (let tickIndex = 0; tickIndex < 2; tickIndex += 1) {
        const beforeX = simulation.world.positionsX[1]!;
        advanceSimulationOneTick(simulation);
        remainders.push(group.dragSpeedRemainder);
        displacements.push(
          simulation.world.positionsX[1]! - beforeX,
        );
        costs.push(getIndividualEnergyActivityInspection(
          combat.individualEnergyActivityStore, 1,
        ).movementExpenditureRequested);
      }
      return { remainders, displacements, costs };
    };

    const fresh = run(100, false);
    const spent = run(0, false);
    const bounded = run(100, true);

    expect(fresh.remainders).toEqual([1, 0]);
    expect(spent.remainders).toEqual(fresh.remainders);
    expect(bounded.remainders).toEqual(fresh.remainders);
    expect(fresh.displacements.map(Math.abs)).toEqual([1, 2]);
    expect(spent.displacements.map(Math.abs)).toEqual([1, 1]);
    expect(bounded.displacements).toEqual([0, 0]);
    expect(bounded.costs).toEqual([0, 0]);
  });

  it("keeps zero-step drag cadence stationary and free until its retained remainder permits walking", () => {
    const simulation = createSimulation(scenario([
      unit(1, 1, 100),
    ...
    group.destinationX = 20;
    group.destinationY = 60;
    const before = Array.from(simulation.world.positionsX);

    advanceSimulationOneTick(simulation);
    expect(Array.from(simulation.world.positionsX)).toEqual(before);
    expect(getIndividualEnergyActivityInspection(
      combat.individualEnergyActivityStore, 1,
    )).toMatchObject({
      requestedPhysicalGait: "stationary",
    ...
    const beforeWalkingTick = simulation.world.positionsX[1]!;

    advanceSimulationOneTick(simulation);
    expect(Math.abs(
      simulation.world.positionsX[1]! - beforeWalkingTick,
    )).toBe(1);
    expect(getIndividualEnergyActivityInspection(
      combat.individualEnergyActivityStore, 1,
    )).toMatchObject({
