...
describe("shared individual physical-gait authority", () => {
  ...
  it("counts bounded sprint ticks to a coordinate or existing contact range", () => {
    expect(requiredPhysicalGaitTicksToCoordinate(0, 0, 24, 0, 4)).toBe(6);
    expect(requiredPhysicalGaitTicksToContact(0, 0, 24, 0, 4, 8)).toBe(4);
    expect(requiredPhysicalGaitTicksToContact(0, 0, 12, 12, 4, 8)).toBe(2);
    expect(requiredPhysicalGaitTicksToContact(0, 0, 8, 0, 4, 8)).toBe(0);
  });
});
